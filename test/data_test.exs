defmodule DDTest do
  ...
  describe "defaults:" do
    ...
    test "can be overridden" do
      with result = A.new(f1: "some", f2: "body") do
        assert result.errors == [ ]
  ...
  describe "fields" do
    ...
    test "that are optional and required are handled" do
      with result = B.new(required: "xxx") do
        assert result.errors == [ ]
  ...
  describe "hidden fields" do
    ...
    test "are categorized properly" do
      with result = D.new(is_hidden: 123, is_visible: "boo") do
        assert result.errors == []
        assert result.values.is_hidden == 123
        assert result.values.is_visible == "boo"

        assert DD.FieldSet.hidden_fields(result) == [ :is_hidden ]
